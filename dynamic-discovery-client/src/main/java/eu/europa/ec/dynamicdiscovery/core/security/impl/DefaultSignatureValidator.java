    ...
    private void verifyCertificate(X509Certificate signerCertificate) throws TechnicalException {
        try {
            ...
            for (String alias : Collections.list(trustStore.aliases())) {
                ...
                //Compare trustedCertificate and certificate generated by the XML Response
                if (!signerCertificate.equals(trustedCertificate)) {
                    continue;
                }

                if (certificateFound != null) {
                    throw new IllegalStateException("TrustStore has more than one trusted certificate as the same.");
                }
